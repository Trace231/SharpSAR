...
if not os.path.exists("./weights/stablesr_000117.ckpt"):
    load_file_from_url(
        url=pretrain_model_url["stablesr_512"],
        model_dir="./weights/",
        progress=True,
        file_name=None,
    )
if not os.path.exists("./weights/stablesr_768v_000139.ckpt"):
    load_file_from_url(
        url=pretrain_model_url["stablesr_768"],
        model_dir="./weights/",
        progress=True,
        file_name=None,
    )
    ...
    try:  # global try
                ...
                with model.ema_scope():
                    ...
                    if init_image.size(-1) < min_size or init_image.size(-2) < min_size:
                        ...
                        rescale = (
                            min_size
                            * 1.0
                            / min(init_image.size(-2), init_image.size(-1))
                        )
                    ...
                    if (
                        init_template.size(-1) <= 1024
                        and init_template.size(-2) <= 1024
                    ):
                        init_latent_generator, enc_fea_lq = vq_model.encode(
                            init_template
                        )
                        init_latent = model.get_first_stage_encoding(
                            init_latent_generator
                        )
                        ...
                        if (
                            init_template.size(-1) <= min_size
                            and init_template.size(-2) <= min_size
                        ):
                            samples, _ = model.sample(
                                cond=semantic_c,
                                struct_cond=init_latent,
                                batch_size=init_template.size(0),
                            ...
                            samples, _ = model.sample_canvas(
                                ...
                                x_T=x_T,
                                return_intermediates=True,
                                tile_size=int(min_size / 8),
                                tile_overlap=min_size // 16,
                                batch_size_sample=init_template.size(0),
                            )
                        ...
                        im_spliter = ImageSpliterTh(
                            init_template,
                            min(init_template.size(-1), init_template.size(-2), 1024),
                            min(
                                init_template.size(-1) - 200,
                                init_template.size(-2) - 200,
                                768,
                            ),
                            sf=1,
                        )
                        for im_lq_pch, index_infos in im_spliter:
                            init_latent = model.get_first_stage_encoding(
                                model.encode_first_stage(im_lq_pch)
                            ...
                            t = repeat(
                                torch.tensor([999]), "1 -> b", b=init_template.size(0)
                            )
                            t = t.to(device).long()
                            x_T = model.q_sample_respace(
                                x_start=init_latent,
        ...
        if rescale > 1:
            x_samples = F.interpolate(
                x_samples,
                size=(int(init_image.size(-2)), int(init_image.size(-1))),
                mode="bicubic",
            )
...
demo = gr.Interface(
    ...
    [
        ...
        gr.inputs.Number(
            default=1, label="Rescaling_Factor (Large images require huge time)"
        ),
        gr.Slider(
            0,
            1,
            value=0.5,
            step=0.01,
            label="CFW_Fidelity (0 for better quality, 1 for better identity)",
        ),
        ...
        gr.Slider(
            ...
            value=200,
            step=1,
            label="Sampling timesteps for DDPM (Large steps for better quality, but huge time)",
        ),
        gr.Dropdown(
